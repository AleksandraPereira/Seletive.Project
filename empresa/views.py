...
def nova_empresa(request):
    if request.method == "GET":
     ...
     return render(request, 'nova_empresa.html', {'techs': techs})
        ...
        if (len(nome.strip()) == 0 or len(email.strip()) == 0 or len(cidade.strip()) == 0 or len(endereço.strip()) == 0 or len(nicho.strip()) == 0 or len(caracteristicas.strip()) == 0 or (not logo)): 
